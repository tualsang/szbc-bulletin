...
export function CurrencyInput({ value, onChange, placeholder = '0.00', ariaLabel }: Props) {
  const preview = formatMoney(value);
  // Only show the preview when it adds information (multi-digit number
  // gets thousands separators) — not for empty input or trivial cases.
  const showPreview = preview && preview !== `$${value}` && parseFloat(value) >= 1000;
